...
## 11. Intégration avatar virtuel (objectif projet LUTIN)

Pour afficher un avatar sur un écran en lieu d'une fenêtre webcam brute, deux approches :

**Option A — Avatar 2D animé (simple)**
- Utiliser `pygame` ou `tkinter` pour superposer un personnage animé sur le flux vidéo
- Synchroniser l'animation de la bouche avec le TTS (`speak()` dans `speech_utils.py`)

**Option B — Avatar 3D (avancé)**
- Utiliser `Blender + BGE`, `Unity + Python socket`, ou `VTubeStudio`
- Exposer un serveur WebSocket dans SMILE qui envoie les événements (parole, émotion)
- L'avatar écoute les événements et anime la bouche / les expressions

Point d'entrée recommandé : envelopper `speak()` dans `speech_utils.py` pour émettre un signal début/fin de parole vers l'avatar.
...
|----------|---------------|----------|
| `ModuleNotFoundError: msvcrt` | Code Windows sur Linux | Appliquer la correction §6.1 |
| Vosk ne transcrit rien | Mauvais modèle ou chemin | Vérifier `VOSK_MODEL_PATH` dans config.py |
| Ollama timeout | Ollama non démarré | `ollama serve &` |
| Webcam non détectée | Index incorrect | Tester `cv2.VideoCapture(1)` |
| Voix TTS en anglais | Voix par défaut | Tester `DEFAULT_VOICE_INDEX = 1` ou installer espeak-ng français |
| Le robot répond en italien | Prompt non traduit | Appliquer §6.2.A |
